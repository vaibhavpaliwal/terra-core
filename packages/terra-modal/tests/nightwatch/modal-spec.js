...
module.exports = resizeTo(['tiny', 'small', 'medium', 'large', 'huge', 'enormous'], {
  'Opens and closes a modal correctly': (browser) => {
    ...
    browser.keys([browser.Keys.ESCAPE])
      .expect.element('[class*="modal-overlay"]').to.not.be.present;
    browser.expect.element('div[role="document"]').to.not.be.present;
  },
  ...
  'Closes a modal on outside click': (browser) => {
    ...
    browser
       .moveToElement('[class*="modal-overlay"]', 235, 5)
       .mouseButtonDown('left')
       .mouseButtonUp('left');

    browser.expect.element('[class*="modal-overlay"]').to.not.be.present;
    browser.expect.element('div[role="document"]').to.not.be.present;
  },
  ...
  'Appends a css class to the overlay': (browser) => {
    browser
      .url(`${browser.launchUrl}/#/tests/modal-tests/append-class`)
      .expect.element('[class*="modal-overlay"]').to.have.attribute('class').which.contains('overlay-custom-class');
  },
